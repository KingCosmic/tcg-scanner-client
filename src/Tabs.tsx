...
import { currentTab, setTab } from './state/routing'
...
function Tab({ Icon, title, index }: any) {
  ...
  useEffect(() => {
    function resize() {
      ...
      const lineWidth = t.offsetWidth + "px";
      t.style.setProperty("--lineWidth", lineWidth);
    }


    function transitionend(e: any) {
      if (e.propertyName != "flex-grow" || 
        !text.current?.classList.contains("active")) return;

      text.current?.classList.add("active");
    }

    text.current?.addEventListener("transitionend", transitionend);

    window.addEventListener('resize', resize)

    return () => {
      text.current?.removeEventListener("transitionend", transitionend);
  ...
  return (
    <button class={`menu__item ${isActive ? 'active' : ''}`} onClick={() => setTab(index)}>
      <div class="menu__icon" >
        <Icon />
      </div>
      <strong class={`menu__text ${isActive ? 'active' : ''}`} ref={text}>{title}</strong>
    </button>
  )
}
...
export default function Tabs() {
  return (
    <menu class="menu">
      {tabs.map(({ Icon, title }, i) => <Tab Icon={Icon} title={title} index={i} />)}
    </menu>
  )
}